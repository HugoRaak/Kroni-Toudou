  ...
  const handleSearchDate = async () => {
    ...
    try {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (
          workMode !== 'Congé' &&
          (conflict.taskMode === 'Tous' || workMode === conflict.taskMode)
        ) {
          foundDate = dateStr;
          break;
        }
  ...
  const handleChangeWorkMode = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        onResolve();
        toast.success(
          `Mode de travail modifié en ${getModeLabel(workMode)} pour le ${formatDate(conflict.taskDate)}`,
        );
      } else {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl max-w-[95vw] overflow-hidden">
        <DialogHeader className="space-y-2">
          ...
          <DialogDescription className="text-xs sm:text-sm break-words leading-relaxed">
            Le mode de la tâche &quot;{taskTitle}&quot; ne correspond pas au mode de travail prévu
            pour le {formatDate(conflict.taskDate)}.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="flex flex-col gap-2 sm:flex-col sm:gap-2">
          ...
          <Button
            ...
            className="w-full text-sm cursor-pointer"
          >
            Définir {formatDateShort(conflict.taskDate)} en{' '}
            {getModeLabel(getWorkdayMode(conflict.taskMode))}
          </Button>
